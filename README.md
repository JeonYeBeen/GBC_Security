# GBC_Security 정리

## Security

## 1. bof 정리
- [bof3 정리](./system/bof3.md)
- [bof4 정리](./system/bof4.md)
- [bof5 정리](./system/bof5.md)
- [bof6 정리](./system/bof6.md)
- [bof7 정리](./system/bof7.md)
- [bof8 정리](./system/bof8.md)

<br>

## 2. 공격 기법 정리
- [ENV 를 이용한 Ret-Sled](./system/ret-sled.md)
- [Got_Overwrite](./system/got%20overwrite.md)
- [RTL](./system/RTL.md)
- [SFP overflow](./system/SFP_Overflow.md)

<br>

## 3. [메모리 보호 기법 정리](./system/Memory_Protect.md)

<br>

## 4. [Resister의 역할](./system/Resister.md)

---

## WEB Security

## 1. 모의 해킹 정리

- SQL Injection
  - [SQL Injection(LOW)](./web/SQL_LOW.md) 
  - [SQL Injection(MID)](./web/SQL_MID.md)
  - [SQL Injection(HIGH)](./web/SQL_HIGH.md)

<Br>

- XSS Stored
  - [XSS Stored(LOW)](web/XSS_LOW.md)
  - [XSS Stored(MID)](web/XSS_MID.md)
  - [XSS Stored(HIGH)](web/XSS_HIGH.md)
  
<br>

- [MySQL 문법](./system/MySQL.md)
- [Web 언어](./system/WEB.md)